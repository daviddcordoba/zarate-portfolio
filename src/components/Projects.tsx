        ...
        <div className='flex flex-wrap justify-center gap-8'>
            {projectsVideos.map((trabajo,id) => 
                ...
                {trabajo.link && trabajo.link.length > 0
                    ? <a target='_blank' className='text-xl text-black max-w-[305px] text-center overflow-hidden hover:underline font-semibold' href={trabajo.link}>{trabajo.name}</a> 
                    : <h2 className='text-xl text-black max-w-[305px] text-center overflow-hidden font-semibold'>{trabajo.name}</h2> 
                }
                </div>
            )}
        </div>   
